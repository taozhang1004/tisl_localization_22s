...
def generate_graphs(data_path:str, f_path:str, scans:dict, num:int, pos_ind:int, feat_ind:int, base:int, method:int, ratio:float, threshold:int) -> int:
    ...
    for scan, label in scans.items():
        ...
        for j in range(num):
            ...
            #sample_v = sample_obj_v
            '''all objects all embeddings (from 4 to 6)'''
            #sample_v = obj_v + emb_v
            '''all objects all embeddings (from 8 to 12)'''
            #sample_emb_v = random.sample(emb_v, min(len(emb_v), random.randint(8, 12)))
            #sample_obj_v = random.sample(obj_v, min(len(obj_v), random.randint(8, 12)))
            #sample_v = sample_emb_v + sample_obj_v
            '''all vlad embeddings'''
            #sample_v = emb_v
            '''all object embeddings'''
            #sample_v = obj_v
            '''all vlad aand object embeddings'''
            sample_v = emb_v + obj_v
