...
# Self added:
cmd_opt.add_argument('-visualize_process', type=bool, default=False, help='whether to visualize the training process')
...
def load_data_test(data:str):

    print('loading test data: {}'.format(data))
    g_list = []
    label_dict = {}
    feat_dict = {}

    with open(join('../experiment_inputs', data), 'r') as f:
        n_g = int(f.readline().strip())
        for i in range(n_g):
            row = f.readline().strip().split()
            n, l = [int(w) for w in row]
