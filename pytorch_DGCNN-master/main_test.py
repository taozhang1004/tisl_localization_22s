...
from util import cmd_args, load_data, load_data_test
import pandas as pd
...
if __name__ == '__main__':
    ...
    train_graphs, valid_graphs = load_data()
    print('# train: %d, # test: %d' % (len(train_graphs), len(valid_graphs)))

    if cmd_args.sortpooling_k <= 1:
        num_nodes_list = sorted([g.num_nodes for g in train_graphs + valid_graphs])
    ...
    for epoch in range(cmd_args.num_epochs):
        ...
        classifier.eval()
        valid_loss = loop_dataset(valid_graphs, classifier, list(range(len(valid_graphs))))
        if not cmd_args.printAUC:
            valid_loss[2] = 0.0
        print('\033[93maverage test of epoch %d: loss %.5f acc %.5f auc %.5f\033[0m' % (epoch, valid_loss[0], valid_loss[1], valid_loss[2]))

        # Record one history
        history.append([avg_loss[0], valid_loss[0], avg_loss[1], valid_loss[1]])
    ...
    if cmd_args.extract_features:
        ...
        features, labels = classifier.output_features(valid_graphs)
        labels = labels.type('torch.FloatTensor')
        np.savetxt('extracted_features_valid.txt', torch.cat([labels.unsqueeze(1), features.cpu()], dim=1).detach().numpy(), '%.4f')

        test_sets = os.listdir('../experiment_inputs')
        for test_set in test_sets:
            test_graphs = load_data_test(test_set)
            features, labels = classifier.output_features(test_graphs)
            labels = labels.type('torch.FloatTensor')
            np.savetxt(join('../experiment_sets', test_set), torch.cat([labels.unsqueeze(1), features.cpu()], dim=1).detach().numpy(), '%.4f')
        

    if cmd_args.visualize_process:
